package com.jet.controller.system.head;
...
import com.jet.controller.base.BaseController;
import com.jet.service.system.appuser.AppuserService;
import com.jet.service.system.user.UserService;
import com.jet.util.AppUtil;
import com.jet.util.Const;
import com.jet.util.PageData;
import com.jet.util.SmsUtil;
import com.jet.util.Tools;
import com.jet.util.Watermark;
import com.jet.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",jet,"+pd.getString("PORT")+",jet,"+pd.getString("EMAIL")+",jet,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",jet,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",jet,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",jet,"+pd.getString("fcontent")+",jet,"+pd.getString("fontSize")+",jet,"+pd.getString("fontX")+",jet,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",jet,"+pd.getString("imgUrl")+",jet,"+pd.getString("imgX")+",jet,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
